...
export default function useMycelRegistry() {
  ...
  const QueryTopLevelDomainAll = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery([key], ({pageParam = 1}: { pageParam?: number}) => {
      ...
      return  client.MycelRegistry.query.queryTopLevelDomainAll(query ?? undefined).then( res => ({...res.data,pageParam}) );
    }, {...options,
  ...
  const QuerySecondLevelDomainAll = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery([key], ({pageParam = 1}: { pageParam?: number}) => {
      ...
      query['pagination.offset']= (pageParam-1)*perPage;
      query['pagination.count_total']= true;
      return  client.MycelRegistry.query.querySecondLevelDomainAll(query ?? undefined).then( res => ({...res.data,pageParam}) );
    }, {...options,
      getNextPageParam: (lastPage, allPages) => { if ((lastPage.pagination?.total ?? 0) >((lastPage.pageParam ?? 0) * perPage)) {return lastPage.pageParam+1 } else {return undefined}},
      getPreviousPageParam: (firstPage, allPages) => { if (firstPage.pageParam==1) { return undefined } else { return firstPage.pageParam-1}}
    }
  ...
  const QueryDomainOwnershipAll = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery([key], ({pageParam = 1}: { pageParam?: number}) => {
      ...
      query['pagination.offset']= (pageParam-1)*perPage;
      query['pagination.count_total']= true;
      return  client.MycelRegistry.query.queryDomainOwnershipAll(query ?? undefined).then( res => ({...res.data,pageParam}) );
    }, {...options,
      getNextPageParam: (lastPage, allPages) => { if ((lastPage.pagination?.total ?? 0) >((lastPage.pageParam ?? 0) * perPage)) {return lastPage.pageParam+1 } else {return undefined}},
      getPreviousPageParam: (firstPage, allPages) => { if (firstPage.pageParam==1) { return undefined } else { return firstPage.pageParam-1}}
    }
  ...
  const QueryDomainRegistrationFee = (name: string, parent: string, query: any, options: any) => {
    ...
    return useQuery([key], () => {
      ...
      return  client.MycelRegistry.query.queryDomainRegistrationFee(name, parent, query ?? undefined).then( res => res.data );
    }, options);
  }
  ...
  return {QueryParams,QueryTopLevelDomain,QueryTopLevelDomainAll,QuerySecondLevelDomain,QuerySecondLevelDomainAll,QueryDomainOwnership,QueryDomainOwnershipAll,QueryDomainRegistrationFee,QueryRole,
  }
}