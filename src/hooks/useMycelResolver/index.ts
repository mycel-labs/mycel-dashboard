...
export default function useMycelResolver() {
  ...
  const QueryWalletRecord = (domainName: string, domainParent: string, walletRecordType: string,  options: any) => {
    ...
    return useQuery([key], () => {
      ...
      return  client.MycelResolver.query.queryWalletRecord(domainName, domainParent, walletRecordType).then( res => res.data );
    }, options);
  }
  ...
  const QueryDnsRecord = (domainName: string, domainParent: string, dnsRecordType: string,  options: any) => {
    ...
    return useQuery([key], () => {
      ...
      return  client.MycelResolver.query.queryDnsRecord(domainName, domainParent, dnsRecordType).then( res => res.data );
    }, options);
  }
